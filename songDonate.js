...
wizard.preview = function(){
	$button_id = "#wizard > article .SONGDONATECOM_CONTAINER > a";
	$button = $($button_id);
	$instance = $button.parent().attr('data-coinwidget-instance');
	$win = $("#SONGDONATECOM_WINDOW_"+$instance);
	...
	$win.find('> span.SONGDONATECOM_COUNT').html($counters.count+'<small>'+$lbl_count+'</small>');
	$win.find('> span.SONGDONATECOM_AMOUNT').html($counters.amount.toFixed(4)+'<small>'+$lbl_amount+'</small>');
	$counter_html = '';
	$right = 'auto';
	switch ($counter) {
		default:
	...
	if (SONGDonateCom.config[$instance]) {
		SONGDonateCom.config[$instance]['alignment'] = $alignment;
		$win.removeClass("SONGDONATECOM_WINDOW_BL SONGDONATECOM_WINDOW_BC SONGDONATECOM_WINDOW_BR SONGDONATECOM_WINDOW_AL SONGDONATECOM_WINDOW_AC SONGDONATECOM_WINDOW_AR");
		$win.addClass("SONGDONATECOM_WINDOW_"+$alignment.toUpperCase());
	}
	$button.parent().parent().css({'top':$top,'left':$left,'right':$right,'margin-left':$margin_left});
	
	$qrcode = ($qrcode == 'on')?true:false;
	if ($qrcode) {
		$win.find('.SONGDONATECOM_QRCODE').show();
	} else {
		$win.find('.SONGDONATECOM_QRCODE').hide();
	}
	...
	$button.find('img').attr('src',SONGDonateCom.source+'icon_'+$currency+'.png');
	$win.find('.COINWIDGET_INPUT_ICON').attr('src',SONGDonateCom.source+'icon_'+$currency+'.png');

	$wallet_val = $wallet;
	if (!$wallet) {
		switch ($currency) {
			default:
			case 'SongCoin': $wallet = SONGDonateCom.config[0].wallet_address; break;
		}
	} 
	if (SONGDonateCom.config[$instance])
		SONGDonateCom.config[$instance]['wallet_address'] = $wallet;
	$uri = $currency+':'+$wallet;
	$win.find('input').val($wallet);
	$win.find('.SONGDONATECOM_WALLETURI').attr('href',$uri);
...
wizard.code = {
	...
	, generate: function(options){
		$code = [];
		$code.push('<script src="http://www.songcoin.org/tipwidget/widget/coin.js"></script>');
		$code.push('<script>');
		$code.push('SONGDonateCom.go({');
	...
	, close: function(){
		$("#code").hide('drop',{direction:'down'},function(){
			$("#wizard").show('drop',{direction:'down'}, function(){
				$("#wizard .SONGDONATECOM_CONTAINER > a").click();
			});	
		});	
	}
	, close_all: function(){
...
wizard.start = function(){
	wizard.restart();
	$(".SONGDONATECOM_WINDOW").each(function(i,v){
		$instance = $(this).attr('id').replace('SONGDONATECOM_WINDOW_','');
		...
		$("#wizard").fadeIn(function(){
			$("#wizard div.loading").delay(500).hide('drop',{direction:'down'},500,function(){
				$("#wizard article").fadeIn(300, function(){
					$("#wizard article .SONGDONATECOM_CONTAINER a").click();
					wizard.preview();
				});
			});
		});
...
wizard.restart = function(){
	...
	$("#wizard article").hide();
	$("#wizard div[data-page='currency'] button[data-currency='btc']").click();
